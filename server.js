...
const addEmployee = () => {
    ...
    let managerIdList = [];
    let roleIdList = [];
    connection.query("SELECT * FROM employees_tracker_db.role", function (err, res) {
        ...
        connection.query("SELECT * FROM employees_tracker_db.employee", function (err, res) {
            if (err) console.log(err);
            for (var i = 0; i < res.length; i++) {
...
const addDepartment = () => {
    ...
    connection.query("SELECT * FROM employees_tracker_db.department", function (err, res) {
        if (err) throw err;
...
const addRole = () => {
    ...
    connection.query("SELECT role.id, title, salary, department_id FROM role LEFT JOIN department ON role.department_id = department.id;", function (err, res) {
        if (err) console.log(err);
        for (var i = 0; i < res.length; i++) {
